    ...
    <div class="container">
        <div class="card mt-4 bg-body-tertiary">
            <div class="card-body text-start">
                <h5 class="card-title">{{ __('Category list') }}</h5>
                <hr>
                ...
                <div class="d-flex justify-content-between">
                    <div>
                        ...
                        {!! Form::select(
                            'status',
                            array_merge(['' => 'All'], [0 => 'inactive', 1 => 'active']),
                            Request::get('status'),
                            [
                ...
                <div class="overflow-y-auto border rounded mb-3 bg-body-secondary" style="height: 35.5vh !important;">
                    <x-table-item-show-component :option="[
                        ...
                        :item-column="[
                            'tableColumnName' => ['Category code', 'Category name', 'Status'],
                            'columnImage' => [],
                            'column' => ['category_code', 'category_name', 'status'],
                            'columnWith' => [],
                            'columnWithIcon' => [],
                        ]" :modal="[
                            'showItem' => [
                    ...
                    @if (!in_array(Request::get('paginate'), ['0']))
                        <div class="btn-group" role="group" aria-label="First group">
                            <a type="button" class="btn btn-sm btn-outline-secondary"
                                ...
                                &laquo; </a>
                            @foreach ($categories->getUrlRange(1, $categories->lastPage()) as $key => $link)
                                <a type="button"
...
@section('modal')
    ...
    <x-modal-item-new-component :modal-title="'Create Category'" :modal-id="'addItemInformationModal'" :modal-size="''" :array-input="['Category name' => 'category_name']"
        :array-textareat="['Category discription' => 'category_description']" :array-select="[
            'Table name' => [
                'name' => 'table_name',
                'data' => array_combine(
                    array_map(function ($value) {
                        return ucfirst(str_replace('_', ' ', $value));
                    }, App\Helpers\DatabaseHelper::getTables()),
                    App\Helpers\DatabaseHelper::getTables(),
                ),
            ],
        ]" :array-checkbox="[]" :route="['name' => 'admin.categories.store', 'method' => 'post']" />
@endsection
            ...
            $('#btnDeleteItem').click(function() {
                ...
                if ($("#formListItem" + " .input-form-checkbox").val() !== '') {
                    ...
                    if ($('#modalNotification').data('type') == "modalDeleteRole") {
                        ...
                        showModal('modalNotification', 'delete cartegory has id: ' + $("#formListItem" +
